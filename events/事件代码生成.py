def generate_hoi4_event_code(event_number):
    ...
    id = SKR_ending.{event_number}
    title = SKR_ending.{event_number}.t
    desc = SKR_ending.{event_number}.desc
    #picture = GFX_event_SKR_ending_{event_number}
    is_triggered_only = yes
    fire_only_once = yes

    option = {{
        name = SKR_ending.{event_number}.a
...
def generate_localization(event_number):
    return f""" SKR_ending.{event_number}.t: ""
 SKR_ending.{event_number}.desc: ""
 SKR_ending.{event_number}.a: ""
"""
#文本自己改
...
def main():
    ...
    if mode == "1":
        output_filename = f"SKR_ending_events_{min_event}_to_{max_event}.txt"
    else:
        output_filename = f"SKR_ending_localization_{min_event}_to_{max_event}.yml"
