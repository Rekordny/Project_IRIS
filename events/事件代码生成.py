def generate_hoi4_event_code(event_number):
    ...
    id = TDL_temp.{event_number}
    title = TDL_temp.{event_number}.t
    desc = TDL_temp.{event_number}.desc
    #picture = GFX_event_TDL_temp_{event_number}
    is_triggered_only = yes
    fire_only_once = yes

    option = {{
        name = TDL_temp.{event_number}.a
...
def generate_localization(event_number):
    return f""" TDL_temp.{event_number}.t: ""
 TDL_temp.{event_number}.desc: ""
 TDL_temp.{event_number}.a: ""
"""
#文本自己改
...
def main():
    ...
    if mode == "1":
        output_filename = f"TDL_temp_events_{min_event}_to_{max_event}.txt"
    else:
        output_filename = f"TDL_temp_localization_{min_event}_to_{max_event}.yml"
