def generate_hoi4_event_code(event_number):
    ...
    id = USF_flavor.{event_number}
    title = USF_flavor.{event_number}.t
    desc = USF_flavor.{event_number}.desc
    #picture = GFX_event_USF_flavor_{event_number}
    is_triggered_only = yes
    fire_only_once = yes

    option = {{
        name = USF_flavor.{event_number}.a
...
def generate_localization(event_number):
    return f""" USF_flavor.{event_number}.t: ""
 USF_flavor.{event_number}.desc: ""
 USF_flavor.{event_number}.a: ""
"""
#文本自己改
...
def main():
    ...
    if mode == "1":
        output_filename = f"USF_flavor_events_{min_event}_to_{max_event}.txt"
    else:
        output_filename = f"USF_flavor_localization_{min_event}_to_{max_event}.yml"
